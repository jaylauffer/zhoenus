...
void ADonutFlyerPawn::BeginPlay()
{
	Super::BeginPlay();

	if (USaveThemAllGameInstance* GameInstance = GetGameInstance<USaveThemAllGameInstance>())
	{
		CollisionThreatScale = GameInstance->donutAggroTuning.CollisionThreatScale;
	}
}
...
void ADonutFlyerPawn::NotifyHit(class UPrimitiveComponent* MyComp, class AActor* Other, class UPrimitiveComponent* OtherComp, bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (OtherComp && OtherComp->IsSimulatingPhysics())
	{
		...
		{
			const float CollisionAggro = push.Size() * CollisionThreatScale;
			if (ADonutFlyerAIController* DonutController = GetController<ADonutFlyerAIController>())
			{
				DonutController->ApplyAggroEvent(pawn, EDonutAggroEventType::CollisionBump, CollisionAggro);
			}

			AZhoenusPlayerState* ps = pawn->GetPlayerState<AZhoenusPlayerState>();
			if (ps != nullptr)
			{
				ps->RecordDonutCollision(CollisionAggro);
			}
		}
	}
	else
